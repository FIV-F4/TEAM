...
def check_achievement_criteria(db_manager, user_id, criteria):
    ...
    for achievement in achievements:
        if achievement[3] == criteria:  # criteria хранится в 4-й колонке
    ...
    def get_topic_progress(self, user_id):
        ...
        for topic in topics:
            ...
            cursor.execute('''
                ...
                JOIN words ON words_groupings.word_id = words.word_id
                WHERE words_groupings.topic_id = ?
            ''', (topic_id,))
            ...
            cursor.execute('''
                SELECT COUNT(*) FROM words_groupings
                JOIN user_progress ON words_groupings.word_id = user_progress.word_id
                WHERE words_groupings.topic_id = ? AND user_progress.user_id = ? AND user_progress.success_counter > 0
            ''', (topic_id, user_id))
...
if __name__ == "__main__":
    ...
    achievements = [
        Achievement(db_manager, "Первый шаг", "Завершил первый урок"),
        Achievement(db_manager, "Полиглот", "Выучил 100 слов"),
        Achievement(db_manager, "Учитель", "Завершил 10 уроков"),
        Achievement(db_manager, "Лингвист", "Выучил все слова из одной темы"),
        Achievement(db_manager, "Испытатель", "Пройдено 5 тестов"),
        Achievement(db_manager, "Ас в тестах", "Набрано более 90% в тесте"),
        Achievement(db_manager, "Регулярность", "Занимался каждый день в течение недели"),
        Achievement(db_manager, "Словарь", "Добавил 50 слов в словарь"),
        Achievement(db_manager, "Словарный запас", "Перевод 10 слов без ошибок"),
        Achievement(db_manager, "Эксперт по темам", "Завершил все уроки по одной теме")
    ]
    ...
    report = progress_school.get_topic_progress(1)
    for topic in report:
