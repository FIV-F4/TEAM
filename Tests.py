...
class BaseLessonScreen:
    def __init__(self, master, topic, test):
        ...
        self.words = []
        self.answered_words = {}  # Словарь для хранения состояния выбора для каждого слова
    ...
    def setup_ui(self):
        ...
        self.prev_button = tk.Button(self.master, text="Предыдущее слово", command=self.prev_word,
                                     font=('Helvetica', 12))  # Сделать кнопку неактивной
...
class WordLessonScreen(BaseLessonScreen):
    ...
    def show_word(self):
        ...
        if self.words:
            ...
            # Проверка, выбрано ли слово ранее
            if self.current_word_index in self.answered_words:
                self.message_label.config(text=self.answered_words[self.current_word_index]['message'])
                return
    ...
    def check_answer(self, chosen, correct, sound_data):
        ...
        self.answered_words[self.current_word_index] = {
            'chosen': chosen,
            'correct': correct,
